...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    try {
        ...
        if (empty($horaire_d) || empty($horaire_f)) {
            throw new Exception("Veuillez saisir un créneau horaire complet.");
        }

        // Validation du format des horaires
        if (!preg_match('/^\d{2}:\d{2}$/', $horaire_d) || !preg_match('/^\d{2}:\d{2}$/', $horaire_f)) {
            throw new Exception("Format d'horaire invalide. Utilisez HH:MM.");
        }

        if (strtotime($horaire_d) >= strtotime($horaire_f)) {
            throw new Exception("L'heure de début doit être strictement avant l'heure de fin.");
        }

        // Recherche du meilleur parking
        $suggestion = $reservationR->getBestParking($horaire_d, $horaire_f);

        if (!$suggestion) {
            throw new Exception("Aucun parking disponible pour ce créneau horaire. Essayez un autre horaire.");
        }
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
       ...
       <form method="post" class="mb-4">
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="time" class="form-control" id="horaire_d" name="horaire_d" 
                           value="<?= htmlspecialchars($_POST['horaire_d'] ?? '') ?>" required>
                </div>
                
                <div class="col-md-6 mb-3">
                    <label for="horaire_f" class="form-label">Heure de fin :</label>
                    <input type="time" class="form-control" id="horaire_f" name="horaire_f" 
                           value="<?= htmlspecialchars($_POST['horaire_f'] ?? '') ?>" required>
                </div>
            </div>
            
            <button type="submit" class="btn btn-primary">
                <i class="bi bi-search"></i> Trouver un parking
            </button>
        </form>
        ...
        <?php if ($suggestion): ?>
            <div class="suggestion-box">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h3 class="mb-0">Notre recommandation</h3>
                    <?php 
                    $scoreClass = '';
                    if ($suggestion['score'] >= 70) {
                        $scoreClass = 'high-score';
                    } elseif ($suggestion['score'] >= 40) {
                    ...
                    <span class="score-badge <?= $scoreClass ?>">
                        Score: <?= htmlspecialchars(round($suggestion['score'], 2)) ?>
                    </span>
                </div>
                
                <div class="row">
                    <div class="col-md-6">
                        <p><strong><i class="bi bi-p-square"></i> Nom :</strong> <?= htmlspecialchars($suggestion['Nom_Parking']) ?></p>
                        <p><strong><i class="bi bi-geo-alt"></i> Adresse :</strong> <?= htmlspecialchars($suggestion['Adresse_Parking']) ?></p>
                        <p><strong><i class="bi bi-clock"></i> Horaires :</strong> 
                            <?= htmlspecialchars($suggestion['Horaire_Ouv']) ?>h - <?= htmlspecialchars($suggestion['Horaire_Ferm']) ?>h
                        </p>
                    </div>
                    <div class="col-md-6">
                        <p><strong><i class="bi bi-123"></i> Places disponibles :</strong> 
                            <span class="<?= $suggestion['Nombre_Dispo'] > 5 ? 'text-success' : 'text-danger' ?>">
                                <?= htmlspecialchars($suggestion['Nombre_Dispo']) ?>
                            </span>
                        </p>
                        <p><strong><i class="bi bi-cash-coin"></i> Tarif :</strong> <?= htmlspecialchars($suggestion['Tarification']) ?></p>
                        <p><strong><i class="bi bi-calendar-check"></i> Abonnement :</strong> <?= htmlspecialchars($suggestion['Abonnement']) ?></p>
                    </div>
                </div>
                
                <div class="mt-3">
                    <form action="AddReservation.php" method="GET" style="display: inline;">
                        <input type="hidden" name="parking_id" value="<?= htmlspecialchars($suggestion['ID_Parking']) ?>">
                        <button type="submit" class="btn btn-success">Réserver ce parking</button>
                    </form>
                    <a href="listParking.php" class="btn btn-outline-secondary ms-2">Voir d'autres options</a>
                </div>
            </div>
        <?php endif; ?>
    </div>

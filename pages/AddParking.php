...
require_once 'C:/xampp/htdocs/gestion parking/libs/qrlib.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (
        ...
        // Ajouter le parking dans la base de données
        ...
        $parking = new Parking(
            ...
            $_POST["Abonnement"],
            $_POST["Tarification"],
            $_POST["etat"] = $_POST["etat"] ?? "actif"
        );
        $parkingP->AjouterParking($parking);

        // Générer les données pour le QR code
        $data = "Nom du Parking: " . $_POST["Nom_Parking"] . "\n";
        $data .= "Adresse: " . $_POST["Adresse_Parking"] . "\n";
        $data .= "Horaires: " . $_POST["Horaire_Ouv"] . "h - " . $_POST["Horaire_Ferm"] . "h\n";
        $data .= "Tarification: " . $_POST["Tarification"] . "\n";

        // Chemin pour enregistrer le QR code
        $qrCodeFile = 'qrcodes/parking_' . uniqid() . '.png';
        ...
        // Générer le QR code
        QRcode::png($data, $qrCodeFile, QR_ECLEVEL_L, 10);

        $success = "Parking ajouté avec succès !";
    } else {
      ...
      <div class="form-container">
        ...
        <?php if ($success): ?>
            ...
            <div class="qr-code">
                ...
                <p><strong style="color: #0a1d37;">Nom du Parking :</strong> <span style="color: #4da6ff;"><?= $_POST["Nom_Parking"] ?></span></p>
                <p><strong style="color: #0a1d37;">Adresse :</strong> <span style="color: #4da6ff;"><?= $_POST["Adresse_Parking"] ?></span></p>
                <p><strong style="color: #0a1d37;">Horaires :</strong> <span style="color: #4da6ff;"><?= $_POST["Horaire_Ouv"] ?>h - <?= $_POST["Horaire_Ferm"] ?>h</span></p>
                <p><strong style="color: #0a1d37;">Tarification :</strong> <span style="color: #4da6ff;"><?= $_POST["Tarification"] ?></span></p>
                <a href="<?= $qrCodeFile ?>" download="QR_Code_Parking.png" class="btn btn-primary mt-3" id="downloadBtn" onclick="redirectToList()">Télécharger le QR Code</a>        <?php else: ?>
        ...
        <form method="POST" id="ParkingForm">
          ...
          <div class="mb-3">
            ...
            <input type="time" class="form-control" name="Horaire_Ouv" id="Horaire_Ouv" min="0" max="23" required>
            <div class="form-feedback" id="Horaire_OuvFeedback"></div>
          </div>

          <div class="mb-3">
            <label class="form-label">Horaire de Fermeture :</label>
            <input type="time" class="form-control" name="Horaire_Ferm" id="Horaire_Ferm" min="0" max="23" required>
...
.confirmation-modal.active {
 ...
 function redirectToList() {
    // Rediriger vers la page "Liste des Parkings" après un court délai
    setTimeout(() => {
      window.location.href = 'listParking.php';
    }, 1000); // Délai de 1 seconde pour s'assurer que le téléchargement commence
  }
